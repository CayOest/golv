...
concept Game = requires(GameT g) {
                   ...
                   { g.is_terminal() } -> std::convertible_to<bool>;
                   { g.value() } -> std::convertible_to<typename GameT::value_type>;
                   { g.is_max() } -> std::convertible_to<bool>;
                   { g.state() } -> std::convertible_to<typename GameT::state_type>;
               };